...
export function TableHeader({ className, children, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return (
    <thead className={cn('bg-slate-900/90 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-800', className)} {...props}>
      {children}
    </thead>
  );
}
...
export function TableHead({ className, children, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th className={cn('px-4 py-3.5 font-semibold select-none', className)} {...props}>
      {children}
    </th>
  );
}
